...
8. **Durable scoped execution**：每个外部副作用归属于持久化任务帧，并绑定 revision、payload fingerprint、idempotency、审批与审计证据；崩溃恢复只能修复目标范围，不能静默重放未知副作用或误伤兄弟任务。
...
```text
L0 Runtime State   task frames、effects、revisions、approvals、receipts
L1 Audit Evidence  hash-chain events、route/egress/ingress、repair lineage
L2 Episodic        run / frame 结果摘要
L3 Semantic        用户确认的稳定事实
L4 Procedural      用户确认的可复用流程与修复模式
```

- L0 / L1 是崩溃恢复和副作用判定的唯一权威来源。
- L2-L4 只能通过 `source_frame_ids`、`source_effect_ids`、`source_event_ids` 和 source revision/hash 引用运行证据。
- Memory 可以辅助后续规划，但不能证明某个外部调用已经成功，不能复用旧 approval，不能授权跨 frame effect，也不能修改 open repair transaction。
- 任何由 Memory 驱动的副作用都必须在 callback 前重新校验实时 policy、approval、scope、revision 和 effect state。
...
prepareEffect(frame, action)          -> EffectIntent
commitEffect(effect, outcome)         -> EffectRecord
recoverRun(runId)                     -> RecoveryDecision
...
prepareToolInvocation(context, tool)  -> EffectIntent
invokePreparedTool(effectId)          -> ReviewedToolResult
```
...
6. Plan + ReAct 执行
   ...
   - `run_step_actions` 与 task frame/effect state 同事务持久化
   - crash / 断电 / 用户中断后，先装载 recovery capsule、对账 unknown outcome、重验策略与 revision，再从最后已接受的 Observation 恢复

7. 可恢复副作用执行
   - 持久化 task-frame lineage 和 branch-local revision
   - 每个 cloud / tool / file-write / export / memory-write effect 有唯一 owner frame
   - effect identity、payload fingerprint、idempotency identity 分离
   - callback 前写 intent，callback 后写 receipt/outcome；中间崩溃进入 `unknown`
   - typed repair verifier 检查 scope、授权、状态、依赖、revision、approval、policy
   - 本地 recovery lease / CAS 防止多个恢复者同时接管
   - 生成 frame/effect/repair 双向 audit packet

8. Capability Gateway / Skills Market（Beta 起）
   - Tool Broker 统一 built-in、local-recipe/Skill 与后续 MCP tools
   - Skill 编排只能引用固定版本、已授权的 Tool capabilities
   - 每次 tool/Skill 调用关联唯一 owned effect、receipt、ingress 与 audit
...
本地 Agent runtime control 不进入 Alpha 核心闭环。Alpha 只做 InternalAgentAdapter；Beta 在修复工作目录、参数、环境继承和隔离问题并通过越权测试后增加 LocalCommandAgentAdapter 的受限 read-only profile；v1.0 再评估 Codex / Claude Code / Aider / Open Interpreter 等外部 Agent adapter。
...
Ogra 采用 SHD-inspired、representation-neutral 的 durable execution contract，
但不把 SHD Python 研究原型作为生产依赖。第一阶段在现有 TypeScript +
SQLite 栈中实现：

- persistent task frames；
- effect ownership 与 effect state machine；
- payload fingerprint 与 stable idempotency key；
- frame/effect revision 与 stale transaction 检测；
- typed repair plan verification；
- recovery capsule 与 unknown-outcome reconciliation；
- local recovery lease / CAS；
- 可从 `run_events` 和运行状态重建的双向 audit indexes。

副作用执行协议：

```text
sanitize / policy / route / approval
  -> persist intent + effect(in_flight) + pre-audit
  -> adapter callback
  -> persist external receipt
  -> independent ingress review
  -> persist outcome + Observation + post-audit
```

外部 callback 与本地 SQLite 无法成为同一原子事务。callback 已生效但本地未确认时，effect 必须记为 `unknown`；恢复时先查询外部 outcome 或用同一 idempotency key 安全重试。adapter 不支持 outcome query / idempotency 时必须 fail closed、replan 或请求用户决定，不能假装 exactly-once。

完整规范和开发顺序见 `docs/plans/10-shd-inspired-durable-execution-runtime.md`。
...
> Ogra 为每次由 Ogra 管理的本机/云端边界穿越记录可审计的范围摘要、哈希、策略、审批和 receipt；敏感原文与 secret 不写入审计。它无法——也不会——记录数据抵达云供应商后，在供应商基础设施内部发生的事。发送范围、发送原因和返回证据可审计；模型内部思考链不在 Ogra 控制范围内。
...
Alpha 只实现 `InternalAgentAdapter`。Beta 在隔离与越权测试通过后增加 `LocalCommandAgentAdapter` 的受限 read-only profile。外部 Agent adapter 进入 v1.0 或更晚版本。
...
## 6.7 Capability Gateway、Tool Broker 与 MCP

Ogra 用一个 `CapabilityGateway` 管理所有受控能力，但保留两条不同生命周期：
...
Tool 是一次 typed capability invocation；Skill 是可复用的提示或编排，
不天然拥有执行权限；A2A 是 task/message/artifact 生命周期，不能伪装成
tool call。两条路径复用 policy、approval、owned effect、receipt、independent
ingress review、audit 和 recovery envelope。

核心规则：

- Agent 只能提交 `toolId + arguments`；workspace、transport、server、secret、
  approval 与执行上下文由 Ogra Core 注入。
- Tool descriptor/schema/version 不可变并按 run/effect pin；server catalog 或
  schema 变化生成待复审的新版本，不能原地热替换。
- callback 前必须完成 schema validation、数据 high-water mark、policy、route、
  payload-bound approval 与 owned `run_effect`，并在执行前重验以防 TOCTOU。
- 外部结果在独立 Ingress Review 接受前不得写入 Observation；结果里的 URI、
  next action 或嵌套工具请求不得自动执行。
- MCP 只是 discovery/invocation transport，不是权限、信任、审计或恢复权威；
  JSON-RPC request id 和 MCP session 也不是 idempotency/outcome evidence。
- MCP 首期只开放 `initialize`、`tools/list`、`tools/call` 和协商后的
  `tools/list_changed`；resources、prompts、roots、sampling、elicitation 默认禁用。
- 本地 stdio 先于 Streamable HTTP/OAuth；远程 MCP 必须等 OS secret store、
  SSRF/redirect/DNS rebinding 防护、auth audience/scope 绑定完成后开放。
- MCP 标准工具默认不支持 outcome query 或幂等；发送后断联进入 `unknown`，
  非幂等调用不得自动 replay。

完整合同、数据模型、威胁模型和阶段门禁见
[11 Tool Broker and MCP Integration Runtime](docs/plans/11-tool-broker-mcp-integration-runtime.md)。
...
9. SHD-inspired durable execution kernel（task frames、effect ownership、revision、idempotency、typed repair verifier、recovery capsule、local lease、audit packet）
10. Tool Broker 合同 + 一个 read-only `knowledge.search` 垂直切片（Alpha 不开放 MCP）
11. 基础 Policy Engine（出境三档 + 入境三档 + Restricted blocked）
12. 脱敏引擎（版本化规则集 + diff 预览 + re-sanitize 循环）
13. Ingress Review Agent（独立进程，regex 层；语义层 v1.0）
14. Quarantine 隔离表与受限沙箱视图
15. Route Decision（含 egress_mode、redaction_rule_version、ingress findings）
16. Append-only local audit trail（hash chain 完整）
17. Data Safety Center v0（含出境模式与 ingress 摘要）
18. AI Governance Center v0（含 egress approval queue 与 ingress incident）
19. 审计导出 endpoint（NDJSON/CSV，policy-gated）
...
- 应用在 callback 前、外部已生效但本地未确认、Observation 提交前等故障点重启后，能通过 recovery capsule 恢复或安全阻断；`unknown` effect 未对账前不得静默重放。
- 并发恢复只能有一个有效 lease holder；越界 repair、陈旧 revision、陈旧 approval 或 payload fingerprint 不匹配必须在 callback 前失败。
- `knowledge.search` 通过 Tool Broker 完成 schema、workspace scope、policy、owned
  effect、receipt、ingress review 和 Observation 全链路，且 MCP 保持禁用。
...
8. PDF / 自动增量索引评估
9. Agent Group：Pipeline + Parallel + Debate，per-step policy/audit
10. interval + continuous 调度，含 iteration/lifetime bounds
11. built-in + declarative local-recipe Skills，经 Tool Broker 固定版本调用
